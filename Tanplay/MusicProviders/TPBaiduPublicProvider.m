...
@synthesize playingSongInfo, playingURL;
...
{
    NSString *strURL = song.songURL;
    NSRange range = [song.songURL rangeOfString:@"xcode="];
    if(range.length <= 0)
    {
        strURL = [NSString stringWithFormat:@"%@?xcode=%@", song.songURL, song.xcode];
    }
...
{
    return [[TPDownloadSongsProvider sharedProvider] addDownloadTask:self.playingURL
                                                                name:self.playingSongInfo.songName 
                                                              artist:self.playingSongInfo.artistName
                                                               album:self.playingSongInfo.albumName];
}
